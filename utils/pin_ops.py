"""
PIN operations utility module.
Handles PIN hashing and verification.
"""
import hashlib
import logging
import re
import time
from db.pin import get_user_pin_hash, save_user_pin as db_save_user_pin
from db.pin_attempt import get_pin_attempt_data, save_pin_attempt_data, reset_pin_attempts, increment_pin_attempt
import traceback

# Configure module logger
logger = logging.getLogger(__name__)

# Lockout periods in seconds after consecutive failures
PIN_LOCKOUT_PERIODS = {
    3: 30,    # 30 seconds after 3 failures
    5: 300,   # 5 minutes after 5 failures
    7: 1800,  # 30 minutes after 7 failures
    10: 7200  # 2 hours after 10 failures
}

def validate_pin_complexity(pin):
    """
    Validate PIN complexity requirements.
    
    Requirements:
    - 4-48 characters
    - Can contain alphanumeric characters and special characters
    - It's the user's responsibility to choose a secure PIN
    - Apes are lazy and wont use PIN if its too hard to remember, even 1111 is better than nothing
    
    Args:
        pin (str): The PIN to validate
        
    Returns:
        tuple: (bool, str) - (is_valid, error_message)
    """
    # Check length
    if not pin or len(pin) < 4:
        return False, "PIN must be at least 6 characters long."
    
    if len(pin) > 48:
        return False, "PIN must be at most 48 characters long."
    
    return True, ""

def can_attempt_pin(user_id):
    """
    Check if a user can attempt to enter a PIN or if they are in a lockout period.
    
    Args:
        user_id: The user ID
        
    Returns:
        tuple: (bool, int) - (can_attempt, remaining_lockout_seconds)
    """
    # Get attempt data from the database
    attempt_data = get_pin_attempt_data(user_id)
    
    # If no attempt data, user can proceed
    if not attempt_data:
        return True, 0
    
    # Check for active lockout
    current_time = time.time()
    last_attempt_time = attempt_data.get('last_attempt_time', 0)
    failure_count = attempt_data.get('failure_count', 0)
    
    # Determine applicable lockout period
    lockout_period = 0
    for threshold, period in sorted(PIN_LOCKOUT_PERIODS.items()):
        if failure_count >= threshold:
            lockout_period = period
    
    # If no lockout applies
    if lockout_period == 0:
        return True, 0
    
    # Check if lockout period has passed
    time_passed = current_time - last_attempt_time
    if time_passed < lockout_period:
        remaining_time = int(lockout_period - time_passed)
        return False, remaining_time
    
    return True, 0

def record_pin_attempt(user_id, success):
    """
    Record a PIN attempt for tracking failed attempts and lockouts.
    
    Args:
        user_id: The user ID
        success (bool): Whether the PIN attempt was successful
    """
    if success:
        # Reset on successful attempt
        reset_pin_attempts(user_id)
    else:
        # Increment failure count on failed attempt
        increment_pin_attempt(user_id)

def hash_pin(pin):
    """
    Create an irreversible hash of a PIN for secure storage.
    
    Args:
        pin (str): The PIN to hash
        
    Returns:
        str: Hexadecimal SHA-256 hash of the PIN
    """
    if not pin:
        return None
        
    # Convert to string and encode to bytes
    pin_bytes = str(pin).encode('utf-8')
    
    # Create SHA-256 hash
    hashed = hashlib.sha256(pin_bytes).hexdigest()
    
    return hashed

def save_user_pin(user_id, pin):
    """
    Hash and save a user's PIN to the database.
    
    Args:
        user_id: The user ID
        pin (str): The PIN to save
        
    Returns:
        tuple: (bool, str) - (success, error_message)
    """
    # Check PIN complexity
    is_valid, error_message = validate_pin_complexity(pin)
    if not is_valid:
        logger.warning(f"Invalid PIN format for user {user_id}: {error_message}")
        return False, error_message
    
    # Hash the PIN before storing
    pin_hash = hash_pin(pin)
    if not pin_hash:
        logger.warning(f"Invalid PIN provided for user {user_id}")
        return False, "Invalid PIN provided"
    
    # Save the hashed PIN to the database
    if db_save_user_pin(user_id, pin_hash):
        # Reset any lockout/attempt counters for this user
        reset_pin_attempts(user_id)
        return True, ""
    else:
        return False, "Failed to save PIN to database"

def verify_pin(user_id, pin):
    """
    Verify if the provided PIN matches the stored PIN for the user.
    Records attempt for lockout tracking.
    
    Args:
        user_id: The user ID
        pin (str): The PIN to verify
        
    Returns:
        bool: True if PIN matches, False otherwise
    """
    # Get the stored PIN hash
    stored_pin_hash = get_user_pin_hash(user_id)
    
    # If no PIN is set, return False
    if not stored_pin_hash:
        return False
        
    # Hash the provided PIN
    provided_pin_hash = hash_pin(pin)
    
    # Compare the hashes
    result = stored_pin_hash == provided_pin_hash
    
    # Record this attempt
    record_pin_attempt(user_id, result)
    
    return result

def has_pin(user_id):
    """
    Check if the user has set a PIN in the database.
    
    Args:
        user_id: The user ID
        
    Returns:
        bool: True if the user has a PIN, False otherwise
    """
    return get_user_pin_hash(user_id) is not None

def update_user_pin(user_id, old_pin, new_pin):
    """
    Update a user's PIN and re-encrypt all sensitive data with the new PIN.
    
    Args:
        user_id: The user ID
        old_pin (str): The user's current PIN
        new_pin (str): The new PIN to set
        
    Returns:
        tuple: (bool, str) - (success, error_message)
    """
    # Verify the old PIN is correct first
    if not verify_pin(user_id, old_pin):
        logger.warning(f"Old PIN verification failed for user {user_id}")
        return False, "Incorrect current PIN"
    
    # Check PIN complexity
    is_valid, error_message = validate_pin_complexity(new_pin)
    if not is_valid:
        logger.warning(f"Invalid new PIN format for user {user_id}: {error_message}")
        return False, error_message
    
    try:
        # Move imports inside the function to avoid circular dependencies
        # 1. Retrieve the mnemonic with the old PIN
        from db.mnemonic import get_user_mnemonic, save_user_mnemonic
        mnemonic = get_user_mnemonic(user_id)
        
        # 2. Retrieve wallet data with the old PIN
        from db.wallet import get_user_wallets, save_user_wallet
        wallets = get_user_wallets(user_id)
        
        # 3. Save the new PIN
        new_pin_hash = hash_pin(new_pin)
        if not db_save_user_pin(user_id, new_pin_hash):
            logger.error(f"Failed to save new PIN for user {user_id}")
            return False, "Failed to save new PIN"
            
        # 4. Re-encrypt and save the mnemonic with the new PIN
        if mnemonic:
            if not save_user_mnemonic(user_id, mnemonic):
                logger.error(f"Failed to re-encrypt mnemonic for user {user_id}")
                # Rollback to old PIN if this fails
                db_save_user_pin(user_id, hash_pin(old_pin))
                return False, "Failed to re-encrypt mnemonic with new PIN"
        
        # 5. Re-encrypt and save each wallet with the new PIN
        if wallets:
            for wallet in wallets:
                wallet_data = wallet.get('data', {})
                wallet_name = wallet.get('name', 'Default')
                if wallet_data and not save_user_wallet(user_id, wallet_data, wallet_name):
                    logger.error(f"Failed to re-encrypt wallet {wallet_name} for user {user_id}")
                    # Rollback to old PIN if this fails
                    db_save_user_pin(user_id, hash_pin(old_pin))
                    return False, f"Failed to re-encrypt wallet {wallet_name} with new PIN"
        
        # Reset PIN attempts after successful update
        reset_pin_attempts(user_id)
        
        logger.info(f"Successfully updated PIN and re-encrypted data for user {user_id}")
        return True, ""
        
    except Exception as e:
        logger.error(f"Error updating PIN for user {user_id}: {e}")
        logger.error(traceback.format_exc())
        # Try to rollback to old PIN
        db_save_user_pin(user_id, hash_pin(old_pin))
        return False, f"Error updating PIN: {str(e)}" 